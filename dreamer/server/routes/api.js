...
//Create a dream & relate to dreamer
          router.put('/dream/relate/:email', async (req, res) => {
            ...
            await DreamCollection.create(req.body, (errors, results1) => {
                ...
                DreamerCollection.findOne({ email: req.params.email }, (errors, results2) => {
...
router.get('/dream/view/:id',(req,res) => {
    ...
    DreamCollection.findOne({_id:req.params.id}, (errors,results)=> {
...
router.put('/dream/:id',(req,res) => {
    ...
    DreamCollection.findOneAndUpdate({_id:req.params.id}, req.body, {new:true}, (errors,results) => {
        errors ? res.send(errors) : res.send(results);
    })
})
...
router.delete('/dream/:id',(req,res) => {
    ...
    DreamCollection.findOneAndDelete({_id:req.params.id}, (errors,results) => {
        errors ? res.send(errors) : res.send(results);
    })
})
